...
const SelectShip = ({ remainingShips, selectedShip, selectShip }: SelectShipProps) => {
  return (
    <div>
      <h2 className="text-center text-2xl lg:text-start">
        {remainingShips.length === 0 ? 'Start the game' : 'Place your ships'}
      </h2>
        ...
        <ul className="mt-6 flex flex-wrap justify-center gap-3 lg:flex-col">
          {remainingShips.map((ship) => {
            ...
            return (
              <li key={ship.name}>
                <button
                  className={`flex gap-1 rounded border border-rose-100 p-2  ${
